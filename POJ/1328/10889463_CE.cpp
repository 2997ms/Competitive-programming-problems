...
int main()
{
	int island,R,count,island_x[1005],island_y[1005],result,left[1005],right[1005],radar,count1,temp,flag=0;
	cin>>island>>R;
	while(island!=0&&R!=0)
	{
		for(count=1;count<=island;count++)
		{
			...
			{
				left[count]=island_x[count]-(int)sqrt(R*R-island_y[count]*island_y[count]);
				right[count]=island_x[count]+(int)sqrt(R*R-island_y[count]*island_y[count]);
			}				
		}
		if(result!=-1)
		{

...
int main()
{
	...
	while (cin >> num)
	{
		...
		for (i = 0; i < num; i++)
		{
			for (j = i + 1; j < num; j++)
			{
				pos_x = node[i].y - node[j].y + node[i].x;
				pos_y = node[i].y - (node[i].x - node[j].x);

				if (!binsearch(pos_x, pos_y))continue;

				pos_x = node[i].y - node[j].y + node[j].x;
				pos_y = node[j].y - (node[i].x - node[j].x) ;

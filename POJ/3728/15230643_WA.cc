...
int getfa(int x)
{
	...
	else
	{
		...
		up[x] = max(max(up[x], up[f[x]]), up_w[x] - pri[x]);
		down[x] = max(max(down[x], down[f[x]]), pri[x] - down_w[x]);

		return f[x];
	}
}

void getffa(int x,int des)
{
	if (des == x)
	{
		return;
	...
	{
		up_w[x] = max(up_w[x], up_w[fa[x]]);
		down_w[x] = min(down_w[x], down_w[fa[x]]);
		up[x] = max(max(up[x], up[fa[x]]), up_w[x] - pri[x]);
		down[x] = max(max(down[x], down[fa[x]]), pri[x] - down_w[x]);

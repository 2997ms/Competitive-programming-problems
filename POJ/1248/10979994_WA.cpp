	...
	while(cin>>target>>s&&target!=0&&!(s[0]=='E'||s[1]=='N'||s[2]=='D'||s[3]=='\0'))
	{
		...
		for(count1=0;count1<len;count1++)
		{
			for(count2=0;count2<len;count2++)
			{
				...
				{
					if(s[count1]!=(s[count3])&&(s[count2])!=(s[count3]))
					{
					for(count4=0;count4<len;count4++)
					{
						if(s[count1]!=(s[count4])&&(s[count2])!=(s[count4])&&(s[count3])!=(s[count4]))
						{
						for(count5=0;count5<len;count5++)
						{
							if(s[count1]!=(s[count5])&&(s[count2])!=(s[count5])&&(s[count3])!=(s[count5])&&(s[count4])!=(s[count5]))
							{
								if((s[count1]-'A'+1)-poww((s[count2]-'A'+1),2)+poww((s[count3]-'A'+1),3)-poww((s[count4]-'A'+1),4)+poww((s[count5]-'A'+1),5)==target)
							{
								sign=1;

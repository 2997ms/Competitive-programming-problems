...
#include <string>
using namespace std;
int panduan(char s[50],char max[50],int count1,int count2,int count3,int count4,int count5)
{
	
	if(s[count1]!=max[1])
	{
			return s[count1]-max[1];
	}
	else if(s[count2]!=max[2])
	{
			return s[count2]-max[2];
	}
		else if(s[count3]!=max[3])
	{
			return s[count3]-max[3];
	}
			else if(s[count4]!=max[4])
	{
			return s[count4]-max[4];
	}
				else if(s[count5]!=max[5])
	{
			return s[count5]-max[5];
	}
}
int main()
{
				...
				for(count3=0;count3<strlen(s);count3++)
				{
					for(count4=0;count4<strlen(s);count4++)
					{
						for(count5=0;count5<strlen(s);count5++)
						{
							...
							{
								sign=1;
								if(panduan(s,max,count1,count2,count3,count4,count5)>=0)
								{
